...
4. **~~プール退場で酩酊の印を失う~~ → 解消(第109 レーン D1・2026-08-12)**: ``world/pool.py``
   の dehydrate が ``_inc_intox_until``(下の :data:`INTOX_KEY`)を運ぶようになった
   (``rumors`` の ``_rumors`` と同じ扱い)。属性が非既定のときだけ退避 dict にキーを足す
   ので、酩酊していない個体・本機構 OFF のランでは退避のバイト列は変わらない。
   写しの一致は ``tests/test_pool_rotation.py`` が機械固定する。
