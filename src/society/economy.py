...
WAGE_CAT: dict[str, str | None] = {
    ...
    # ---- ペルソナプール v2(全年齢化。docs/plans/persona-pool-v2-plan.md §3-3)----
    # ★v2 は L1(住民)に 0-14 歳 9.96% と 65 歳以上 18.77% を実在化させる。そこで初めて
    #   「賃金労働をしない年齢層」が名簿に現れるので、**明示的に無給(None)と宣言**する。
    #   宣言しないと `wage_eligible` が True を返し、勤務窓さえ付けば賃金プランが生える
    #   (= 未就学児に月給が出る)という静かなバグになる。
    # ★保育士・幼稚園教諭・調理員は**ここへ入れない**。教員・看護師と同じく org 配属を
    #   持つ被用者なので、`wage_profile`(産業 × 規模帯 × 職種群)が払う経路が正しい。
    #   WAGE_CAT に入れると `wage_eligible` の分岐は変わらないが、粗い定額へ落ちてしまう。
    "未就学児": None,                     # 0-5 歳。保育所・幼稚園に居るが賃金の受け手ではない
    "年金生活者": None,                   # 65+ の非労働力「その他」= 引退。年金は経済 v0 に無い
}
...
MONEY_INIT: dict[str, tuple[int, int]] = {
    ...
    "車掌": (40000, 120000),
    # ---- ペルソナプール v2(全年齢化)----
    "未就学児": (100, 2000),                # おこづかい相当。★下限 0 は路上生活者だけの区分
    "年金生活者": (30000, 120000),          # 年金 + 貯蓄の取り崩しで暮らす層(高齢の金融資産は厚い)
}
...
OCC_GROUP_RULES: tuple[tuple[str, str], ...] = (
    ...
    ("講師", "instructor"),
    # ---- 専門職 ----
    # ★ペルソナプール v2 の新語 3 つ(保育士 / 幼稚園教諭 / 調理員)。国家資格職なので
    #   professional。ここに置かないと OCC_GROUP_RULES の既定 "general"(倍率 1.00)へ
    #   黙って落ち、保育士が総合職と同じ賃金水準になる(= 静かな不整合)。
    #   ★「調理員」は下の service 節に置く(調理師=有資格 とは別区分。無資格の給食調理)。
    ("保育士", "professional"), ("幼稚園教諭", "professional"),
    ...
    # ---- 接客・販売・補助 ----
    ("調理員", "service"),                 # ★v2: 保育所の給食調理(無資格)。調理師 とは別
