...
正直な限界(5 件)
------------------
- **解析側の分類はまだ入れていない**(IF-E の監視装置が正しく鳴る側に倒してある)。
  ``economy_sfc`` の**閉じた不変量**(Σ 全主体残高 + RoW + K5)には ``city_total`` の
  ``lost`` 項を足して閉じたが、``scripts/analyze_accounting.py`` の ``MONEY_KINDS`` /
  部門フロー分類器には ``lost_return`` / ``lost_keep`` / ``lost_expire`` を**まだ足していない**。
  したがって同スクリプトの ``unclassified_money_kinds`` にこの 3 種が
  「新しい金の経路」として**正直に列挙される**(= 監視装置が設計どおり鳴っている状態)。
  部門行列に載せるには家計 → 家計の内部移転と「遺失物」という主体なしバケツの扱いを
  決める必要があり、それは会計側の設計判断なので本レーンでは踏み込まない。
