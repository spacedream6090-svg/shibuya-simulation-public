...
FEATURES: tuple[Feature, ...] = (
    ...
    # PPF 夜間計画プリフェッチ(2026-08-25 ユーザー要望「計画は夜のうちに作って処理を分散」)。
    # 実装 src/society/cognition/plan_prefetch.py。
    # strict の根拠: 候補の選抜も順序も **既存フィールドの整数比較だけ**(plan_day / plan_step /
    #   sleeping / loc / reflect_step / sleep_until / id)で、LLM の自由文を 1 バイトも読まず
    #   乱数 stream を 1 本も引かない。生成そのものは起床時と同一の既存経路を通る。
    # ★affects_k=True を正直に宣言する: generate() の**発生点(step)そのものを動かす**。
    #   総数は増えない(1 暦日 1 計画のガードは触っていない)が、「いつ撃つか」は
    #   予算の取り合いを通じて同 step の他の呼(発話・返答)の granted 率に効きうる。
    #   間接経路ではなく明示的な作用点なので True(day_plan.boundary と同じ扱い)。
    # fingerprint_risk=none: **プロンプトを 1 バイトも変えない**(前倒しするのは時刻だけで、
    #   計画プロンプトの構築経路は起床時とまったく同一)。
    # 併記キー(bool でないので個別宣言は不要): start_min(既定 10=00:10)/ end_min(既定
    #   350=05:50)= 夜間の窓。窓外の step では述語が 1 回も走らない。
    _f("planning.day_plan.prefetch.enabled", "strict", True, "none",
       "夜間計画プリフェッチ。シミュ内 00:10-05:50 の夜間 step で、まだ当日の計画を持たない"
       "**就寝中**の個体の計画を予算の余りで先行生成する(順序=起床予定の早い順)。"
       "塞ぐ穴の実測: 起床の 54% が 04:40 の 1 step に 34,019 人・全需要の 92% が "
       "04:40-09:59 に集中し、FIFO 繰り越しが約 10 時間ぶん滞留する一方、00:10-04:30 の "
       "夜間 step では life レーンがほぼ空で回っている。成立根拠は「当日の日付・天気・予定・"
       "計画の日スタンプがシミュ内 0 時に確定済み」であること。★generate() の呼び出し点を"
       "1 つも足さない(1 個体 1 暦日 ≤ 1 本のガードは不触・予算の総量も 1 も動かない)。"
       "ただし計画が立つ時刻が変われば当日の行動も変わる = 世界の軌道が分岐するので、"
       "ラン全体の総呼数は ON/OFF で一致しない(ラン境界では前倒しのぶん ON が先行する = "
       "RFX-A と同型の ±1 日効果)。★候補は毎 step 再計算する"
       "純粋な述語 = 新しい checkpoint 状態ゼロ(resume==straight が構造的に成り立つ)。"
       "★L1 は plan_created の prefetch:true 1 欄のみ(ON のときだけ・記録専用)。"
       "★正直な限界: 記憶参照(mem.retrieve)の step と行間ダイジェストの消費点が前へずれる /"
       "内省の完了は『予約が残っていない』で近似する(完了マーカーがリポジトリに無い)"),
